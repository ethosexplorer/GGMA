/**
 * Import All Government, Enforcement, Executive, and Legislative Authorities (All 50 States + DC)
 */
...
// 1. Precise State Regulators (combining seed data + APPLICATIONS spreadsheet)
const REGULATORS = [
  ...
  { state: 'KS', name: 'Kansas Department of Agriculture', email: 'kda.info@ks.gov', phone: '785-564-6700' },
...
// 3. State Police / Highway Patrol (All 50 States)
const STATE_POLICE = {
...
// 4. State Legislature Senates (All 50 States)
const SENATES = {
...
// 5. Federal Agencies and Regional DEA Offices
const FEDERAL_DEA = [
...
async function run() {
  console.log('🏛️  Starting Seeding of All Government, Enforcement, and Policy Presets...');
  ...
  // Function to save a contact card in Firestore
  const saveContact = async (org) => {
    const docId = `gov-${org.state.toLowerCase()}-${org.type}-${slugify(org.name)}`;
    ...
    await setDoc(ref, {
      ...
      city: org.city || '',
      state: org.state,
      jurisdiction: STATE_NAMES[org.state] || org.state,
      type: org.type, // specific subtype
      email: org.email || '',
      phone: org.phone || '',
      licenseStatus: 'Active',
      source: 'Official Government Registry / Sheet',
      status: 'Lead',
      pipeline: 'new',
      tags: [org.state.toLowerCase(), org.type, 'authority', 'policy'],
      notes: org.notes || '',
      emailVerified: true, // Mark verified so it is ready to blast immediately
    ...
    seededCount++;
    if (seededCount % 50 === 0) {
      console.log(`   ... Seeded ${seededCount} records`);
    }
  };

  // Process Regulator list
  ...
  for (const [state, email] of Object.entries(GOVERNORS)) {
    ...
    await saveContact({
      name: `Governor of ${stateName} (${state})`,
      state: state,
      type: 'governor',
      email: email,
  ...
  // Process State Police / Highway Patrol
  ...
  console.log(`   Seeded: ${seededCount} new contacts`);
