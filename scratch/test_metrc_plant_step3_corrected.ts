...
async function createPackagePlantsStep3Corrected() {
  ...
  const plantLabel = 'AAA0E0000000D13000000991';

  const credentials = btoa(`${integratorKey}:${userKey}`);

  try {
    console.log('Running Plants Step 3 (Create Package from Plant) with corrected fields...');
    
    // v2/plantbatch/packages uses 'PlantLabel', 'PackageTag', 'PlantBatchType', 'Count', 'Location', 'Item'
    const body = [{
...
createPackagePlantsStep3Corrected();
