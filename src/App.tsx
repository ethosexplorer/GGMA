          ...
          {[
            ...
            <div key={i} className="flex items-center justify-between p-4 border border-slate-100 rounded-xl">
              ...
              <div className="flex gap-2">
                <button onClick={() => { import('./lib/turso').then(function(m) { m.turso.execute({ sql: 'INSERT INTO audit_logs (id, action, user_id, data) VALUES (?, ?, ?, ?)', args: ['log-' + Math.random().toString(36).substr(2, 9), 'UI_Action', 'Production_User', JSON.stringify({ detail: 'Action executed' })] }).catch(function(e) { console.error(e) }) }) }} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors"><XCircle size={18} /></button>
                <button onClick={() => { import('./lib/turso').then(function(m) { m.turso.execute({ sql: 'INSERT INTO audit_logs (id, action, user_id, data) VALUES (?, ?, ?, ?)', args: ['log-' + Math.random().toString(36).substr(2, 9), 'UI_Action', 'Production_User', JSON.stringify({ detail: 'Action executed' })] }).catch(function(e) { console.error(e) }) }) }} className="p-2 text-emerald-500 hover:bg-emerald-50 rounded-lg transition-colors"><CheckCircle size={18} /></button>
              </div>
            </div>
          ))}
        </div>
    ...
    <div className="min-h-screen bg-slate-50 overflow-x-hidden">
      ...
      <section className="py-24 px-6 bg-white overflow-hidden border-t border-slate-200">
        <div className="max-w-6xl mx-auto">
          ...
          {/* GGMA National Regulatory & Enforcement Tracker */}
          <div className="mt-16 bg-slate-900 text-white rounded-3xl p-8 max-w-4xl mx-auto shadow-xl border border-slate-800 relative overflow-hidden">
            {/* Design accents */}
            ...
            <div className="relative z-10 space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-6">
                <div>
                  <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full bg-amber-500/10 text-amber-400 text-[10px] font-black uppercase tracking-widest border border-amber-500/20 mb-2">
                    ⚖️ Regulatory Transparency Audit
                  </div>
                  <h3 className="text-2xl font-black text-white tracking-tight flex items-center gap-2">
                    GGMA National Regulatory & Enforcement Tracker
                  </h3>
                  <p className="text-slate-400 text-xs mt-1 max-w-xl font-medium">
                    State regulators often do not maintain compiled, historical record logs of license suspensions, forfeitures, and enforcement reasons. GGMA aggregates regulatory actions nationwide in real-time.
                  </p>
                </div>
                <div className="flex items-center gap-2 bg-slate-950 px-3 py-1.5 rounded-xl border border-slate-800/80 shrink-0 self-start md:self-center">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                  <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">Live Cloud Database</span>
                </div>
              </div>

              {/* Real-time Search Input / Controls */}
              <div className="flex flex-col sm:flex-row gap-3">
                <div className="flex-1 relative">
                  <input
                    type="text"
                    placeholder="Search by business name, DBA, or license number..."
                    value={ommaSearchQuery}
                    onChange={(e) => handleOmmaSearch(e.target.value)}
                    className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500/50 rounded-xl py-3 pl-4 pr-10 text-sm font-semibold outline-none transition-all placeholder:text-slate-600"
                  />
                  {ommaSearchQuery && (
                    <button
                      onClick={() => handleOmmaSearch('')}
                ...
                <div className="sm:w-48 bg-slate-950 rounded-xl border border-slate-800 flex items-center px-3">
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-wider mr-2">State:</span>
                  <select
                    className="flex-1 bg-transparent border-none text-xs font-bold text-white outline-none cursor-pointer py-3"
                    onChange={(e) => {
                      const val = e.target.value;
                      if (!ommaSearchQuery && val !== 'ALL') {
                        // Seed query with state code to trigger list filtering
                        handleOmmaSearch(val);
                      } else if (ommaSearchQuery) {
                        // Re-run search with current text or filter
                        handleOmmaSearch(ommaSearchQuery);
                      }
                    }}
                  >
                    <option value="ALL" className="bg-slate-950">All States</option>
                    <option value="OK" className="bg-slate-950">Oklahoma (OK)</option>
                    <option value="MS" className="bg-slate-950">Mississippi (MS)</option>
                    <option value="CA" className="bg-slate-950">California (CA)</option>
                    <option value="TX" className="bg-slate-950">Texas (TX)</option>
                    <option value="MO" className="bg-slate-950">Missouri (MO)</option>
                  </select>
                </div>
              </div>
              ...
              <div className="space-y-3 pt-2">
                {ommaSearchLoading ? (
                  <div className="flex items-center justify-center py-8 gap-3 text-slate-500 text-xs font-bold">
                    <div className="w-5 h-5 rounded-full border-2 border-slate-700 border-t-emerald-500 animate-spin" />
                    Querying live regulatory ledger...
                  </div>
                ) : ommaSearchQuery.trim() ? (
                  ommaSearchResults.length > 0 ? (
                    ommaSearchResults.map((record) => (
                      <div key={record.id} className="bg-slate-950/60 border border-slate-800/80 rounded-2xl p-5 hover:border-slate-700 transition-all flex flex-col md:flex-row justify-between gap-4">
                        <div className="space-y-2">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="px-2 py-0.5 rounded bg-emerald-950 text-emerald-400 text-[10px] font-black border border-emerald-900/50 uppercase tracking-widest">{record.state}</span>
                            <span className="text-sm font-black text-white">{record.business_name}</span>
                            {record.dba && <span className="text-slate-500 text-xs font-semibold">({record.dba})</span>}
                          </div>
                          <p className="text-xs text-slate-400 font-semibold leading-relaxed">
                            <strong className="text-slate-500">Reason:</strong> {record.reasons}
                          </p>
                          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                            <span>License Type: {record.license_type}</span>
                            <span>•</span>
                            <span>License: {record.license_number}</span>
                          </div>
                        </div>
                        <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-2 shrink-0 border-t md:border-t-0 border-slate-800/60 pt-3 md:pt-0">
                          ...
                          <span className="text-[9px] text-slate-500 font-black tracking-widest uppercase">{record.dates_connected}</span>
                        </div>
                      </div>
                    ))
                  ) : (
                    <div className="text-center py-8 text-slate-500 text-xs font-semibold bg-slate-950/20 rounded-2xl border border-dashed border-slate-800">
                      No matching records found for "{ommaSearchQuery}" in our compliance index.
                    </div>
                  )
                ) : (
                  // Default list showing recent state enforcement actions
                  <div className="space-y-3">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2 mb-3">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                      Recently Synced National Audits (Live Feed)
                    </p>
                    {recentEnforcements.length > 0 ? (
                      recentEnforcements.map((record) => (
                        <div key={record.id} className="bg-slate-950/30 border border-slate-800/40 rounded-2xl p-4 flex flex-col md:flex-row justify-between gap-3 hover:border-slate-800 transition-colors">
                          <div className="space-y-1">
                            <div className="flex items-center gap-2">
                              <span className="px-1.5 py-0.5 rounded bg-slate-800 text-slate-300 text-[9px] font-black border border-slate-700 uppercase tracking-wider">{record.state}</span>
                              <span className="text-xs font-black text-white">{record.business_name}</span>
                            </div>
                            <p className="text-[11px] text-slate-400 font-medium leading-relaxed max-w-2xl line-clamp-1">
                              {record.reasons}
                            </p>
                          </div>
                          <div className="flex items-center justify-between md:justify-end gap-3 shrink-0">
                            <span className="text-[9px] text-slate-500 font-black uppercase tracking-wider">{record.dates_connected}</span>
                      ...
                      <div className="text-center py-6 text-slate-600 text-xs font-semibold">
                        No recent enforcements found in index.
                      </div>
                    )}
                  </div>
                )}
...
export const LarryMedCardChatbot = ({ onNavigate, onProfileCreated, variant = 'med-card', userProfile, jurisdiction = 'Oklahoma', activeRole }: any) => {
  ...
  const handleSend = async (e?: React.FormEvent, overrideText?: string) => {
    ...
    if (lower.includes('subscription plan') || lower.includes('view subscription') || lower.includes('state authority plan') || lower.includes('pricing') || lower.includes('basic subscription') || lower.includes('professional subscription') || lower.includes('enterprise subscription')) {
      ...
      if (isBusiness || variant === 'business' || variant === 'sinc') {
        response = '✅ **GGHP Business Subscription Plans**\n\n' +
          ...
          '_All Business tiers include a 14-Day Free Trial and 30% off your first month after the trial._\n\n' +
        ...
        response = '⚖️ **GGHP Attorney Marketplace Plans**\n\n' +
          ...
          '_All Attorney tiers include a 14-Day Free Trial._\n\n' +
        ...
        response = '⭐ **GGHP Professional Suite Subscription Plans**\n\n' +
          ...
          '_All Professional tiers include a 14-Day Free Trial and 30% off your first month after the trial._\n\n' +
        ...
        response = '✅ **GGHP Subscription Plans**\n\n' +
          ...
          '_All tiers include a 14-Day Free Trial._\n\n' +
    ...
    if (lower === 'request policy brief' || lower === 'view economic impact data' || lower === 'regulatory integration inquiry') {
      response = `🏛️ **Government & Policy Access Required**\n\nThe GGHP Ecosystem generates real-time economic impact reports, granular policy analytics, and municipal tax-yield forecasting models at both the **state and national levels**.\n\nDue to the sensitive nature of regulatory data, federal compliance metrics, and interstate economic trends, **unrestricted access is reserved for verified government officials and policy makers.**\n\nBy registering an official account, your office gains access to an extensive suite of tools, including:\n• **National & State Economic Impact Dashboards**\n• **Multi-Jurisdictional Tax & Revenue Forecasting**\n• **Real-Time Regulatory Diversion Tracking**\n• **C³ Patient & Business Compliance Aggregates**\n• **Public Health & Environmental Impact Analytics**\n• **L.A.R.R.Y AI Legislative Compliance Scanning**\n• **Interstate Seed-to-Sale Data Auditing**\n\nTo view these restricted reports and full data sets, please create your official account.`;
      setMessages(prev => [...prev, { role: 'bot', text: response, choices: ['Secure Login / Sign Up', 'Main Menu'] } as any]);
      setSignupStep(0);
      setIsTyping(false);
      return;
    ...
    if (lower.includes('ggma') || lower.includes('omma')) {
      ...
      setMessages(prev => [...prev, { 
        role: 'bot', 
        text: response,
        choices: ['Account & Login', 'Application Status', 'Billing Question', 'How Does This Work?', 'Speak with a Live Agent', 'Main Menu'] 
      } as any]);
      setSignupStep(800);
      setIsTyping(false);
      return;
      ...
      // Telehealth Options
      if ((window as any).Calendly) { (window as any).Calendly.initPopupWidget({ url: 'https://calendly.com/globalgreenhpmeet/health-wellness-consultation' }); } else { window.open('https://calendly.com/globalgreenhpmeet/health-wellness-consultation', '_blank'); }
      response = '📅 **Booking page opened!** If it didn\'t open, click below:\n🔗 [Book Medical Card / Doctor Visit via Calendly](https://calendly.com/globalgreenhpmeet/health-wellness-consultation)\n\n';
      setMessages(prev => [...prev, { role: 'bot', text: response, choices: ['Main Menu'] } as any]);
      setSignupStep(0);
      setIsTyping(false);
      return;
      ...
      setIsTyping(false);
      return;
    } else if (lower.includes('Speak with a Live Agent') || lower.includes('human') || lower.includes('political') || lower.includes('media') || lower.includes('press') || lower.includes('enforcement') || lower.includes('state authority') || lower.includes('federal')) {
      response = `👩‍💼 **Direct Escalation to Management**\n\nI have priority-routed this to the next available senior representative. I have priority-routed this directly to the priority queue (Flagged: 🟣 Priority Purple). She has been notified of the urgency and will reach out to you promptly.\n\nWould you like to return to the Main Menu?`;
      setMessages(prev => [...prev, { role: 'bot', text: response, choices: ['Main Menu'] } as any]);
      setSignupStep(0);
      setIsTyping(false);
      return;
    ...
    if (foundState) {
    ...
    } else if (lower.includes('change state') || lower.includes('move state') || lower.includes('transfer state') || lower.includes('change my state') || lower.includes('transfer jurisdiction')) {
      response = '🔒 **Jurisdiction Locked**\n\nFor regulatory compliance, your profile is permanently bound to your verified state. To transfer your operations to a new jurisdiction, I must temporarily lock your active licenses and submit a transfer request to our Human Compliance Reviewers. \n\nShall we proceed? (Yes to proceed / No to cancel)';
      setMessages(prev => [...prev, { role: 'bot', text: response, choices: ['Yes', 'No'] } as any]);
      setSignupStep(888); // special step for state transfer
      setIsTyping(false);
      return;
    } else if (signupStep === 888) {
      if (lower.includes('yes') || lower.includes('proceed')) {
         response = 'Understood. I have flagged your account for a Jurisdiction Transfer. A human reviewer will contact you shortly to verify your new state ID and documentation. Your account is now in Transfer Pending status.';
         setMessages(prev => [...prev, { role: 'bot', text: response, choices: ['Main Menu'] } as any]);
         setSignupStep(0);
      } else {
         response = 'Jurisdiction Transfer cancelled. Your current state licenses remain active.';
         setMessages(prev => [...prev, { role: 'bot', text: response, choices: ['Main Menu'] } as any]);
         setSignupStep(0);
      }
      setIsTyping(false);
      ...
      if (lower.includes('yes') || lower.includes('book') || lower.includes('ready') || lower.includes('consultation')) {
        if ((window as any).Calendly) { (window as any).Calendly.initPopupWidget({ url: 'https://calendly.com/globalgreenhpmeet/health-wellness-consultation' }); } else { window.open('https://calendly.com/globalgreenhpmeet/health-wellness-consultation', '_blank'); }
        response = 'Sure! I\'ve opened the booking page. If it didn\'t open, click below:\n\n🔗 [Book Telehealth Session](https://calendly.com/globalgreenhpmeet/health-wellness-consultation)\n\n';
      ...
      response = '⚕️ **Physician Booking & Patient Licensing**\n\nWhat type of visit do you need?\n\n• **Medical Card Evaluation ($45)** — physician recommendation for OMMA card (Call: 1-888-963-4447)\n• **General Telehealth Wellness** — non-emergency consultation (Call: 1-888-963-4447)\n\nSelect below to proceed to online booking! 📅';
      setMessages(prev => [...prev, { 
        role: 'bot', 
        text: response,
  ...
  const quickActions = isBusiness
    ...
    : [
        { label: '📅 Book Medical Intake', text: 'https://book.carepatron.com/Diversity-Health---Wellness-Network--GoHealthUSA---CCardz-/live agent-R-?p=MeBev6pvQWuqD4djocNXFg&s=cOEr6HSN' },
              ...
              <div className="flex-1 max-w-[85%] bg-white border border-slate-200/80 rounded-2xl rounded-bl-md shadow-sm p-4">
                ...
                {bookingError === 'AUTH_REQUIRED' && (
                  <div className="space-y-4">
                    ...
                    <button
                      onClick={() => {
                        const link = isBusiness ? 'https://calendly.com/globalgreenhpmeet/health-wellness-consultation' : 'https://calendly.com/globalgreenhpmeet/health-wellness-consultation';
                        if ((window as any).Calendly) { (window as any).Calendly.initPopupWidget({ url: link }); } else { window.open(link, '_blank'); }
                      }}
