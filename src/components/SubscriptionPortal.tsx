              ...
              {(liveAction.type === 'warning' || liveAction.type === 'form') && (
                <button onClick={() => {
                  ...
                  import('../lib/turso').then(({ turso }) => {
                    const auditId = 'BILLING-' + Math.random().toString(36).substr(2, 9).toUpperCase();
                    turso.execute({
                      sql: "INSERT INTO audit_logs (id, action, user_id, data) VALUES (?, ?, ?, ?)",
                      args: [auditId, liveAction.title, 'BILLING_User', JSON.stringify({ detail: liveAction.message, type: liveAction.type })]
                    }).then(() => {
