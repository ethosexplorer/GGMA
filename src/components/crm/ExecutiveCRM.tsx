...
export const ExecutiveCRM = ({ 
  ...
  useEffect(() => {
    ...
    const mapDoc = (d: any, col: string): Deal => {
      ...
      return { id: d.id, collection: col, stage: data.stage || 'lead', ...data, name: data.name || data.businessName || 'Unnamed', contactName: data.contactName || '', phone: data.phone || '', email: data.email || '', value: data.value ?? 0, assignedTo: data.assignedTo || 'unassigned', type, jurisdiction: data.jurisdiction || data.state || '' } as any;
    };
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-xs text-slate-500 font-medium mb-1 flex flex-col gap-0.5">
                        ...
                        {!deal.email && !deal.phone && !deal.contactName && <span className="italic text-slate-400">No contact specfied</span>}
