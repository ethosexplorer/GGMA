...
const SEED_EVENTS: CalEvent[] = [
  { id: '1', title: 'New Registration Assistance', date: '2026-05-03', startTime: '10:00', endTime: '10:30', category: 'admin_support', color: 'bg-pink-500', attendees: 'asstsupport@gmail.com', meetLink: 'https://calendly.com/asstsupport' },
];
...
export const AdminSupportCalendar = () => {
  ...
  const [form, setForm] = useState({ title: '', date: '', startTime: '09:00', endTime: '10:00', category: 'admin_support', description: '', attendees: 'asstsupport@gmail.com', location: '', meetLink: 'https://calendly.com/asstsupport' });
  ...
  const addEvent = () => {
    ...
    setShowForm(false);
    setForm({ title: '', date: '', startTime: '09:00', endTime: '10:00', category: 'admin_support', description: '', attendees: 'asstsupport@gmail.com', location: '', meetLink: 'https://calendly.com/asstsupport' });
  };
            ...
            {monthDays.map(({ date, inMonth }, i) => {
              ...
              return (
                <div key={i} onClick={() => { setSelectedDate(ds); setView('day'); }}
                  className={cn("min-h-[100px] border-b border-r border-slate-50 p-2 cursor-pointer transition-all hover:bg-pink-50/50",
                    !inMonth && "bg-slate-50/50 opacity-50", isSelected && "ring-2 ring-inset ring-pink-500/30 bg-pink-50/30", isToday && "bg-emerald-50/50")}>
          ...
          <div className="grid grid-cols-8 border-b border-slate-100">
            ...
            {weekDays.map(d => {
              ...
              return (<div key={ds} onClick={() => { setSelectedDate(ds); setView('day'); }}
                className={cn("py-3 text-center cursor-pointer hover:bg-pink-50/50", isToday && "bg-emerald-50")}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            <div className="bg-white border border-slate-200 rounded-3xl shadow-sm p-6">
              <h4 className="font-black text-slate-800 mb-4 flex items-center gap-2"><CalIcon size={16} className="text-pink-600" /> Events for {new Date(selectedDate + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</h4>
            ...
            <button onClick={() => { setForm(f => ({ ...f, date: selectedDate })); setShowForm(true); }} className="w-full py-3 border-2 border-dashed border-slate-200 rounded-2xl text-sm font-bold text-slate-400 hover:border-pink-400 hover:text-pink-600 transition-all flex items-center justify-center gap-2"><Plus size={16} /> Add Event</button>
          </div>
        </div>
      )}
    </div>
