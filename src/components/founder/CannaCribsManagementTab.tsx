...
export const CannaCribsManagementTab = () => {
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-4 gap-2">
            {[{ label: 'Total Properties', value: String(properties.length), change: `${properties.filter(p => p.status === 'Occupied').length} occupied`, icon: Home, color: 'text-green-600', bg: 'bg-green-50' },{ label: 'Active Tenants', value: String(properties.filter(p => p.status === 'Occupied').length), change: `${properties.length > 0 ? Math.round((properties.filter(p => p.status === 'Occupied').length / properties.length) * 100) : 0}% occupancy`, icon: Users, color: 'text-blue-600', bg: 'bg-blue-50' },{ label: 'Pending Apps', value: String(applications.filter(a => a.status === 'Pending Review').length), change: 'Review needed', icon: ClipboardList, color: 'text-amber-600', bg: 'bg-amber-50' },{ label: 'Monthly Revenue', value: `$${totalTierRevenue.toLocaleString()}`, change: `${landlords.length} landlords`, icon: DollarSign, color: 'text-emerald-600', bg: 'bg-emerald-50' }].map((k, i) => (
              <div key={i} className="bg-white rounded-lg border border-slate-200 p-3 hover:shadow-md transition-all">
                <div className="flex items-center gap-2">
                  <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center shrink-0', k.bg)}><k.icon size={15} className={k.color} /></div>
                  <div>
                    <div className="text-lg font-black text-slate-900 leading-tight">{k.value}</div>
                    <div className="text-[10px] text-slate-500 font-medium">{k.label}</div>
                    <div className="text-[9px] text-emerald-600 font-bold">{k.change}</div>
                  </div>
                </div>
              </div>
            ))}
          ...
          {activePartners.length > 0 && (
            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-lg p-3 text-white">
              <div className="flex items-center justify-between mb-2">
                <h3 className="font-black text-xs flex items-center gap-1.5"><Shield size={13} /> Insurance Affiliate Revenue</h3>
                <button onClick={() => setSubTab('insurance')} className="px-2 py-1 bg-white/20 hover:bg-white/30 text-[10px] font-bold rounded transition-all flex items-center gap-1"><BarChart3 size={10} /> Full Dashboard</button>
              </div>
              <div className="grid grid-cols-4 gap-2">
                <div className="bg-white/10 backdrop-blur rounded-md p-2 text-center"><div className="text-base font-black">{referralStats.totalClicks}</div><div className="text-[9px] text-white/70 font-bold">Total Clicks</div></div>
                <div className="bg-white/10 backdrop-blur rounded-md p-2 text-center"><div className="text-base font-black">{referralStats.quoted}</div><div className="text-[9px] text-white/70 font-bold">Quoted</div></div>
                <div className="bg-white/10 backdrop-blur rounded-md p-2 text-center"><div className="text-base font-black">{referralStats.converted}</div><div className="text-[9px] text-white/70 font-bold">Converted</div></div>
                <div className="bg-white/10 backdrop-blur rounded-md p-2 text-center"><div className="text-base font-black">{activePartners.length}</div><div className="text-[9px] text-white/70 font-bold">Partners</div></div>
              </div>
            </div>
          )}

          {/* Fee Schedule — Editable */}
          <div className="bg-white rounded-lg border border-slate-200 p-4"><h3 className="font-bold text-slate-900 text-xs mb-2 flex items-center gap-2"><CreditCard size={14} className="text-violet-600" />Tenant & Guest Fee Schedule <span className="text-[8px] text-emerald-500 bg-emerald-50 px-1.5 py-0.5 rounded-full font-bold ml-1">Click to edit</span></h3><div className="grid grid-cols-4 gap-2">{feeEntries.map(([key, fee]) => (<div key={key} className="py-2 px-2 bg-slate-50 rounded text-center group cursor-pointer hover:bg-green-50 transition-colors" onClick={() => { if (editingFeeKey !== key) { setEditingFeeKey(key); setEditingFeeVal(fee.amount); } }}>{editingFeeKey === key ? (<div className="space-y-1" onClick={e => e.stopPropagation()}><input autoFocus value={editingFeeVal} onChange={e => setEditingFeeVal(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') saveFeeEdit(key, editingFeeVal); if (e.key === 'Escape') setEditingFeeKey(null); }} className="w-full px-2 py-0.5 text-center text-sm font-black text-green-600 border border-green-300 rounded outline-none focus:ring-2 focus:ring-green-500/20" /><div className="flex gap-1"><button onClick={() => saveFeeEdit(key, editingFeeVal)} className="flex-1 text-[9px] font-bold text-white bg-green-600 rounded px-1 py-0.5">Save</button><button onClick={() => setEditingFeeKey(null)} className="text-[9px] font-bold text-slate-500 bg-slate-200 rounded px-1 py-0.5">Cancel</button></div></div>) : (<><div className="text-sm font-black text-green-600 group-hover:text-green-700">{fee.amount}</div><div className="text-[10px] font-bold text-slate-700">{fee.label}</div><div className="text-[9px] text-slate-400">{fee.type}</div></>)}</div>))}{feeEntries.length === 0 && <p className="col-span-4 text-xs text-slate-400 text-center py-2">Fee schedule loading...</p>}</div></div>
        </div>
      )}
      ...
      <div className="mt-6 pt-4 border-t border-slate-200 flex items-center justify-center gap-2 pb-2">
        <div className="h-px flex-1 bg-gradient-to-r from-transparent to-slate-200" />
        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
          <Leaf size={10} className="text-green-500" /> End of CannaCribs • {subTab.charAt(0).toUpperCase() + subTab.slice(1)}
        </span>
