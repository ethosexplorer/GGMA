...
export const RoleSelectorScreen = ({ userProfile, onSelect, onLogout, onHome }: { userProfile: any, onSelect: (role: string) => void, onLogout?: () => void, onHome?: () => void }) => {
  ...
  const allRoles = [
    ...
    { id: 'admin_internal', label: 'Internal Team (Management)', desc: 'HR, Personnel, Applications & Oversight', icon: Briefcase, color: 'teal' },
    { id: 'operations', label: 'Operations Staff (Call Center)', desc: 'Call Center, Phone Intake, Support & CannaCribs', icon: Headset, color: 'cyan' },
