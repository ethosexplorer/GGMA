...
export const RoleSelectorScreen = ({ userProfile, onSelect, onLogout, onHome }: { userProfile: any, onSelect: (role: string) => void, onLogout?: () => void, onHome?: () => void }) => {
  ...
  const allRoles = [
    ...
    { id: 'admin_internal', label: 'Internal Team (Management)', desc: 'LVL 4 — HR, Personnel, Applications & Oversight', icon: Briefcase, color: 'teal' },
    { id: 'operations', label: 'Operations Staff (Call Center)', desc: 'LVL 2 — Call Center, Phone Intake, Support Queue', icon: Headset, color: 'cyan' },
