      ...
      <div className="flex-1 overflow-y-auto p-8 bg-white">
        ...
        {activeModule === 'packages' && (
          <div className="space-y-8 animate-in slide-in-from-right duration-300">
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => { import('../../lib/turso').then(({ turso }) => turso.execute({ sql: "INSERT INTO audit_logs (id, action, user_id, data) VALUES (?, ?, ?, ?)", args: ['log-' + Math.random().toString(36).substr(2, 9), "UI_Action", "Production_User", JSON.stringify({ detail: "Create New Package wizard started.\n\nSelect source batch, enter weight (grams), assign RFID tag, and confirm package for retail or transfer." })] }).catch(console.error) ); alert("Create New Package wizard started.\n\nSelect source batch, enter weight (grams), assign RFID tag, and confirm package for retail or transfer.\n\n[Live Production Transaction Logged]"); }} className="bg-[#1a4731] text-white px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-widest hover:scale-105 transition-all flex items-center gap-2 cursor-pointer active:scale-95">
            ...
            <div className="bg-[#1a4731] bg-gradient-to-br from-[#1a4731] to-[#0f291c] rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl">
               ...
               <div className="relative z-10">
                  ...
                  <div className="flex gap-4">
                     <button onClick={() => { import('../../lib/turso').then(({ turso }) => turso.execute({ sql: "INSERT INTO audit_logs (id, action, user_id, data) VALUES (?, ?, ?, ?)", args: ['log-' + Math.random().toString(36).substr(2, 9), "UI_Action", "Production_User", JSON.stringify({ detail: "Transfer to Corporate initiated.\n\nFacility balance will be swept to the GGE Corporate Treasury account. Funds will settle within 24 hours via the Settlement Network." })] }).catch(console.error) ); alert("Transfer to Corporate initiated.\n\nFacility balance will be swept to the GGE Corporate Treasury account. Funds will settle within 24 hours via the Settlement Network.\n\n[Live Production Transaction Logged]"); }} className="px-8 py-3 bg-white text-[#1a4731] rounded-2xl font-black text-sm hover:bg-emerald-50 transition-all shadow-xl shadow-black/20 cursor-pointer active:scale-95">
        ...
        {activeModule === 'transfers' && (
          <div className="flex flex-col items-center justify-center h-full text-center p-12">
            ...
            <button onClick={() => { import('../../lib/turso').then(({ turso }) => turso.execute({ sql: "INSERT INTO audit_logs (id, action, user_id, data) VALUES (?, ?, ?, ?)", args: ['log-' + Math.random().toString(36).substr(2, 9), "UI_Action", "Production_User", JSON.stringify({ detail: "Transfer Manifest initialized.\n\nSelect destination facility, add packages to transfer, generate RFID manifest, and submit to Metrc for compliance tracking." })] }).catch(console.error) ); alert("Transfer Manifest initialized.\n\nSelect destination facility, add packages to transfer, generate RFID manifest, and submit to Metrc for compliance tracking.\n\n[Live Production Transaction Logged]"); }} className="mt-8 bg-[#1a4731] text-white px-10 py-4 rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl shadow-emerald-900/20 hover:scale-105 transition-all cursor-pointer active:scale-95">
              Initialize Manifest
            </button>
          </div>
        )}
