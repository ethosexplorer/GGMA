...
async function runNevadaScraper() {
  console.log("🚀 Starting Nevada (CCB) Cannabis Registry Scraper...");
  ...
  try {
    console.log("🌐 Navigating to Nevada CCB Portal...");
    await page.goto("https://ccb.nv.gov/list-of-licensees/", { 
    ...
    while (extractedCount < limit) {
      const records = await page.evaluate(() => {
        ...
        for (const container of containers) {
          ...
          if (text.includes('Active') || text.includes('NV') || text.match(/\\d{3}[-\\.\\s]?\\d{4}/)) {
             results.push(text);
          }
        }
        return results;
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        const business = {
          ...
          "License Number": extractMatch(/([0-9]{10,20})/i) !== "N/A" ? extractMatch(/([0-9]{10,20})/i) : "Unknown",
    ...
    if (businesses.length > 0) {
      ...
      const csvPath = path.join(process.cwd(), 'nv_enriched_directory.csv');
