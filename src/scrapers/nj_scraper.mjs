...
async function runNewJerseyScraper() {
  console.log("🚀 Starting New Jersey (CRC) Cannabis Registry Scraper...");
  ...
  try {
    console.log("🌐 Navigating to New Jersey CRC Portal...");
    await page.goto("https://www.nj.gov/cannabis/businesses/recreational/", { 
    ...
    while (extractedCount < limit) {
      const records = await page.evaluate(() => {
        ...
        for (const container of containers) {
          ...
          if (text.includes('Active') || text.includes('NJ') || text.match(/\\d{3}[-\\.\\s]?\\d{4}/)) {
             results.push(text);
          }
        }
        return results;
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        const business = {
          ...
          "License Number": extractMatch(/([0-9]{8,12})/i) !== "N/A" ? extractMatch(/([0-9]{8,12})/i) : "Unknown",
    ...
    if (businesses.length > 0) {
      ...
      const csvPath = path.join(process.cwd(), 'nj_enriched_directory.csv');
