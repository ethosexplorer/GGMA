...
async function runArizonaScraper() {
  console.log("🚀 Starting Arizona (ADHS) Cannabis Registry Scraper...");
  ...
  try {
    console.log("🌐 Navigating to Arizona ADHS Portal...");
    await page.goto("https://azdhs.gov/licensing/medical-marijuana/index.php", { 
    ...
    while (extractedCount < limit) {
      const records = await page.evaluate(() => {
        ...
        for (const container of containers) {
          ...
          if (text.includes('Active') || text.includes('AZ') || text.match(/\\d{3}[-\\.\\s]?\\d{4}/)) {
             results.push(text);
          }
        }
        return results;
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        const business = {
          ...
          "License Number": extractMatch(/([0-9]{10})/i) !== "N/A" ? extractMatch(/([0-9]{10})/i) : "Unknown",
    ...
    if (businesses.length > 0) {
      ...
      const csvPath = path.join(process.cwd(), 'az_enriched_directory.csv');
