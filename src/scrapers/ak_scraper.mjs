...
async function runAlaskaScraper() {
  console.log("🚀 Starting Alaska AMCO Licensing Portal Scraper...");
  const browser = await puppeteer.launch({ headless: "new", defaultViewport: { width: 1280, height: 800 } });
  const page = await browser.newPage();
  const businesses = [];

  try {
    await page.goto("https://www.commerce.alaska.gov/web/amco/MarijuanaLicenseDatabase.aspx", { waitUntil: 'networkidle2', timeout: 60000 });
    await new Promise(r => setTimeout(r, 3000));
    const records = await page.evaluate(() => {
      const items = Array.from(document.querySelectorAll('tr, p, li'));
      return items.map(item => item.innerText || '').filter(t => t.includes('License') || t.includes('Store') || t.includes('Cultivation'));
    });
    ...
    if (businesses.length > 0) {
      ...
      const csvPath = path.join(process.cwd(), 'ak_scraped_directory.csv');
