...
import { Users, Briefcase, MessageSquare, BarChart3, Building2, Phone, Clock, Activity, Shield,
  CircleCheck, AlertTriangle, Target, TrendingUp, Globe, FileText, CreditCard, FolderOpen, Send,
  Plus, Loader2, BookOpen } from 'lucide-react';
...
export const ManagerDashboard = ({ onLogout, user }: { onLogout?: () => void, user?: any }) => {
  ...
  const saveCallLog = async () => {
    ...
    try {
      await turso.execute({ sql: "INSERT INTO audit_logs (id, action, user_id, data) VALUES (?, ?, ?, ?)", args: [`clog-${Date.now()}`, 'CALL_LOG', managerName, JSON.stringify({ category: callLogCategory, notes: callLogNotes, time: new Date().toISOString() })] });
  ...
  const createTicket = async () => {
    ...
    try {
      await turso.execute({ sql: "INSERT INTO compliance_alerts (id, entity_id, message, severity, status, is_resolved, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)", args: [`tkt-${Date.now()}`, 'mgr-created', `[${managerName}] ${newTicketMsg}`, newTicketSev, 'Open', 0, new Date().toISOString()] });
  ...
  const renderHome = () => (
    <div className="space-y-6">
      <div className="bg-gradient-to-br from-slate-900 via-sky-900 to-slate-900 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
        <div className="absolute top-0 right-0 p-6 opacity-10"><Building2 size={100}/></div>
        <div className="relative z-10">
          <h2 className="text-2xl font-black tracking-tight">Manager Console, {managerName.split(' ')[0]}</h2>
          <p className="text-sky-200 mt-1 text-sm">Regional oversight, team lead management, and critical interventions.</p>
        </div>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Queue', value: liveQueue, color: 'text-rose-500' },
          { label: 'Teams', value: teamLeads.length || '3', color: 'text-sky-500' },
          { label: 'Escalations', value: escalations.length, color: escalations.length > 0 ? 'text-red-500' : 'text-emerald-500' },
          { label: 'Calls Today', value: recentCalls.length, color: 'text-blue-500' },
        ].map((s, i) => (
      ...
      {escalations.slice(0,3).map((a: any, i: number) => (
        <div key={i} className="p-3 rounded-xl border-l-4 bg-red-50 border-red-500 shadow-sm">
          <p className="font-bold text-slate-800 text-sm">{a.message}</p>
          <p className="text-[10px] text-slate-400 mt-1">Critical • {a.created_at}</p>
        </div>
      ))}
    </div>
  );
  ...
  const renderTeams = () => (
    <div className="space-y-6">
      <h2 className="text-xl font-black text-slate-800">My Teams</h2>
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <table className="w-full text-sm text-left">
          <thead className="bg-slate-50 border-b border-slate-100"><tr>
            <th className="px-5 py-3 font-black text-slate-500 text-[10px] uppercase tracking-widest">Team Lead</th>
            <th className="px-5 py-3 font-black text-slate-500 text-[10px] uppercase tracking-widest">Size</th>
            <th className="px-5 py-3 font-black text-slate-500 text-[10px] uppercase tracking-widest">Status</th>
          </tr></thead>
          <tbody className="divide-y divide-slate-50">
            <tr className="hover:bg-slate-50"><td className="px-5 py-3 font-bold text-slate-800">Sarah Jenkins (Alpha)</td><td className="px-5 py-3">8 Reps</td><td className="px-5 py-3"><span className="text-xs font-bold text-emerald-500">On Track</span></td></tr>
            <tr className="hover:bg-slate-50"><td className="px-5 py-3 font-bold text-slate-800">Marcus Chen (Beta)</td><td className="px-5 py-3">6 Reps</td><td className="px-5 py-3"><span className="text-xs font-bold text-amber-500">Below Target</span></td></tr>
          </tbody>
        </table>
      </div>
    </div>
  ...
  const renderEscalations = () => (
    <div className="space-y-5">
      <h2 className="text-xl font-black text-slate-800">Director-Level Escalations</h2>
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 space-y-3">
        {escalations.length > 0 ? escalations.map((a: any, i: number) => (
          <div key={i} className="p-4 border border-red-200 bg-red-50 rounded-xl flex justify-between items-center">
            <div><p className="font-bold text-slate-800 text-sm">{a.message}</p><p className="text-[10px] text-slate-500 mt-1">Critical • {a.created_at}</p></div>
            <div className="flex gap-2">
              ...
              <button className="px-3 py-1.5 bg-slate-900 text-white text-[10px] font-black rounded-lg" onClick={() => { turso.execute({ sql: "INSERT INTO compliance_alerts (id, entity_id, message, severity, status, is_resolved, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)", args: [`esc-founder-${Date.now()}`, a.entity_id, `[ESCALATED by MANAGER] ${a.message}`, 'CRITICAL', 'Escalated to President', 0, new Date().toISOString()] }).catch(console.error); alert('Escalated to Founder.'); turso.execute({ sql: 'UPDATE compliance_alerts SET is_resolved = 1 WHERE id = ?', args: [a.id] }).then(() => setEscalations(prev => prev.filter(x => x.id !== a.id))); }}>Escalate to Founder</button>
            </div>
          </div>
        )) : <div className="p-8 text-center"><CircleCheck size={32} className="mx-auto text-emerald-500 mb-2"/><p className="text-slate-400 text-sm">No critical escalations.</p></div>}
      </div>
    </div>
  );
      ...
      <div className="w-56 flex flex-col shrink-0 hidden md:flex bg-slate-950 border-r border-slate-900">
        <div className="p-4 pb-2">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center shadow-lg text-white bg-sky-600"><Globe size={18}/></div>
            <div><h2 className="font-black text-xs text-white uppercase tracking-tight">Manager</h2><p className="text-[9px] font-black tracking-widest uppercase text-sky-400">{managerName.split(' ')[0]}</p></div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-4 space-y-0.5">
          {NAV.map(item => (
            <button key={item.id} onClick={() => setTab(item.id)} className={cn("w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-bold transition-all text-left", tab === item.id ? "bg-sky-600 text-white shadow-lg" : "text-slate-400 hover:bg-white/5 hover:text-slate-100")}>
