...
const ChiefComplianceDirectorDashboard = ({ user, onLogout }: { user?: any, onLogout?: () => void }) => {
  ...
  const [notifications, setNotifications] = useState(() => {
    try {
      const dismissedIds = JSON.parse(localStorage.getItem('gghp_dismissed_compliance_notifications') || '[]');
      if (Array.isArray(dismissedIds)) {
        return INITIAL_NOTIFICATIONS.filter(n => !dismissedIds.includes(n.id));
      }
    } catch (e) {
      console.error('Error loading dismissed compliance notifications', e);
    }
  ...
  useEffect(() => {
    try {
      ...
      const dismissedIds = INITIAL_NOTIFICATIONS
        ...
        .map(n => n.id);
      localStorage.setItem('gghp_dismissed_compliance_notifications', JSON.stringify(dismissedIds));
    } catch (e) {
      console.error('Error saving dismissed compliance notifications', e);
    }
  }, [notifications]);
