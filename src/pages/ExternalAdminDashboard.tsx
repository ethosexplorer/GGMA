...
export const ExternalAdminDashboard = ({ onLogout, user }: { onLogout?: () => void, user?: any }) => {
      ...
      <div className="bg-emerald-50 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 p-4 rounded-xl flex items-start md:items-center justify-between gap-4 flex-col md:flex-row">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-emerald-700">"Welcome back to {entityName} Admin â€” part of the Global Green Hybrid Platform (GGHP). You have 3 pending renewals and 1 anomaly at Westside Hub. Would you like me to prepare them or escalate to Executive review?"</p>
          </div>
        </div>
  ...
  const getContent = () => {
    switch (activeTab) {
      ...
      case 'documents': return renderModulePlaceholder('Documents & Uploads', 'Upload and view required documents â€” licenses, inspections, and compliance records.', Folder);
    ...
    <div className="flex h-screen overflow-hidden bg-slate-50 text-slate-800 font-sans relative">
      ...
      <div className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col hidden md:flex shrink-0 transition-all duration-500">
        ...
        <div className="p-3 mx-3 mb-3 bg-slate-800/50 border border-slate-700 rounded-lg">
          <p className="text-[10px] text-slate-500 leading-relaxed"><span className="text-slate-400 font-bold">ðŸ”’ Scoped Access:</span> You can only see data for {entityName}. System-wide data, enforcement logic, and financial backend are not accessible.</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col h-[calc(100vh)] overflow-hidden bg-slate-50 transition-all duration-500">
        <div className="h-14 border-b border-slate-200 flex items-center justify-between px-6 bg-white shrink-0">
          <div className="flex items-center gap-3">
            <h1 className="text-lg font-bold text-slate-800 tracking-tight">Admin Dashboard â€“ {entityName}</h1>
