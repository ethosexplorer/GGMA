...
export const StateAuthorityDashboard = ({ onLogout, user }: { onLogout?: () => void, user?: any }) => {
  ...
  // ═══════════════════════════════════════════════════════════════
  //  TAB: STATEWIDE OVERVIEW — Economic Hero + OMES/OMMA Hierarchy
  // ═══════════════════════════════════════════════════════════════
  const renderStatewideOverview = () => (
    <div className="space-y-6 animate-in fade-in duration-500">
      {/* OMES / OMMA GOVERNANCE HIERARCHY */}
          ...
          <div className="flex flex-col md:flex-row items-center gap-6 mb-8">
            <div className="flex-1 bg-white/5 border border-white/10 rounded-2xl p-6 relative group hover:border-indigo-500/30 transition-all">
              ...
              <div className="flex items-center gap-4 mb-3">
                ...
                <div>
                  <h3 className="text-xl font-black text-white">OMES</h3>
            ...
            <div className="flex-1 bg-emerald-500/5 border border-emerald-500/20 rounded-2xl p-6 relative group hover:border-emerald-500/40 transition-all ring-1 ring-emerald-500/10">
              ...
              <div className="flex items-center gap-4 mb-3">
                ...
                <div>
                  <h3 className="text-xl font-black text-white">{stateData?.licensingAuthority || 'OMMA'}</h3>
                  <p className="text-xs text-emerald-300 font-bold">{stateJurisdiction} Cannabis Authority</p>
                </div>
              </div>
              <p className="text-[11px] text-slate-400 leading-relaxed">Licensing • Compliance • Inspections • Patient registry • Seed-to-sale tracking via {stateData?.traceabilitySystem || 'Metrc'}</p>
            </div>
          </div>

          {/* Economic Snapshot KPIs */}
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {[
              { label: 'Active Licenses', value: '4,281', sub: '+142 this quarter', color: 'text-white', icon: FileCheck },
              { label: 'Annual Tax Revenue', value: '$148.2M', sub: '7% excise collected', color: 'text-emerald-400', icon: DollarSign },
              { label: 'Jobs Created', value: '12,847', sub: 'Direct + indirect', color: 'text-blue-400', icon: Users },
              { label: 'Economic Multiplier', value: '3.4x', sub: 'Per $1 invested', color: 'text-amber-400', icon: TrendingUp },
              { label: 'Patient Cards Active', value: '218,400', sub: stateData?.reciprocity ? '✅ Reciprocity' : 'State residents', color: 'text-cyan-400', icon: HeartPulse },
              { label: 'Compliance Rate', value: '96.8%', sub: 'Statewide average', color: 'text-violet-400', icon: ShieldCheck },
            ].map((kpi, i) => (
      ...
      {/* INDUSTRY VERTICALS ACTIVE IN STATE */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { vertical: 'Cannabis', authority: stateData?.licensingAuthority || 'OMMA', status: stateData?.cannabisStatus || 'Medical Only', statusColor: getCannabisStatusColor(stateData?.cannabisStatus || 'Medical Only'), icon: Leaf, gradient: 'from-emerald-600 to-teal-700', border: 'border-emerald-500/30', licenses: '4,281' },
          { vertical: 'Hemp / Agriculture', authority: 'ODAFF', status: stateData?.hempProgram ? 'Active Program' : 'No Program', statusColor: stateData?.hempProgram ? 'text-emerald-400' : 'text-slate-500', icon: Factory, gradient: 'from-green-700 to-lime-800', border: 'border-green-500/30', licenses: '892' },
          { vertical: 'Alcohol & Beverage', authority: stateData?.alcoholAuthority || 'ABLE Commission', status: 'Regulated', statusColor: 'text-amber-400', icon: Store, gradient: 'from-amber-700 to-orange-800', border: 'border-amber-500/30', licenses: '6,420' },
          { vertical: 'Pharmaceuticals', authority: stateData?.pharmaBoard || 'OK Board of Pharmacy', status: 'Regulated', statusColor: 'text-cyan-400', icon: FlaskConical, gradient: 'from-cyan-700 to-blue-800', border: 'border-cyan-500/30', licenses: '3,210' },
        ].map((v, i) => (
        ...
        <div className="flex flex-col md:flex-row gap-6 items-start md:items-center relative z-10">
          <div className="flex-1">
            ...
            <p className="text-sm text-teal-100/90 leading-relaxed max-w-4xl">
              The U.S. Department of Justice issued a final order to reclassify marijuana at the federal level. {stateData?.licensingAuthority || 'OMMA'} is actively monitoring this development. The move from Schedule I to Schedule III expands research opportunities and could unlock new state commerce channels.
            </p>
          </div>
      ...
      {/* REGULATORY SHIFTS + COMPLIANCE PULSE */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
          ...
          <div className="space-y-4">
            {[
              { t: 'Emergency Rule #882', d: 'Updated packaging requirements for edibles across all dispensaries.', s: 'Effective Now', c: 'text-emerald-600 bg-emerald-50' },
              { t: 'Amendment SB-104', d: 'Expansion of reciprocity for TX/MO patients — opens interstate commerce.', s: 'Pending Sign', c: 'text-amber-600 bg-amber-50' },
              { t: 'OMES Procurement RFP #2026-041', d: 'Enterprise cannabis technology platform — statewide licensing & compliance.', s: 'Open', c: 'text-indigo-600 bg-indigo-50' },
              { t: 'Compliance Update', d: 'New seed-to-sale reporting frequency (Daily) via Metrc integration.', s: 'Effective May 1', c: 'text-blue-600 bg-blue-50' },
            ].map((rule, i) => (
        ...
        <div className="bg-slate-900 rounded-2xl p-8 text-white relative overflow-hidden border border-slate-800">
          ...
          <div className="space-y-6">
            {[
              { label: 'License Verification Rate', value: '99.4%', pct: 99.4, color: 'bg-emerald-500', text: 'text-emerald-400' },
              { label: 'Audit Completion (Q2)', value: '72%', pct: 72, color: 'bg-indigo-500', text: 'text-indigo-400' },
              { label: 'Inspection Pass Rate', value: '91.2%', pct: 91.2, color: 'bg-blue-500', text: 'text-blue-400' },
              { label: 'Metrc Sync Compliance', value: '97.6%', pct: 97.6, color: 'bg-cyan-500', text: 'text-cyan-400' },
            ].map((bar, i) => (
              <div key={i}>
                ...
                <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                  <div className={cn("h-full rounded-full transition-all duration-1000", bar.color)} style={{ width: `${bar.pct}%` }}></div>
                </div>
              </div>
            ))}
            <div className="pt-4 p-4 bg-white/5 rounded-xl border border-white/5">
              <p className="text-xs text-slate-400 font-medium leading-relaxed italic">
                "State system operating under GGHP Oversight protocols. {stateData?.licensingAuthority || 'OMMA'} monitoring 4,281 active licenses. 0 critical security breaches detected."
              </p>
            </div>
          </div>
        </div>
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-gradient-to-br from-slate-900 to-indigo-950 p-8 rounded-3xl border border-slate-700/50 text-white relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-slate-400 font-medium mb-8">How the regulated cannabis industry contributes to the state economy across employment, tax revenue, and GDP.</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
              ...
              <p className="text-4xl font-black text-white">$1.82B</p>
              <p className="text-xs text-slate-400 mt-2 font-bold">2.4% of state GDP • +18% YoY growth</p>
              <div className="mt-4 h-2 bg-white/5 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-emerald-500 to-teal-400 rounded-full" style={{ width: '24%' }}></div>
              </div>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
              <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-2">Total Tax Revenue (Annual)</p>
              <p className="text-4xl font-black text-white">$148.2M</p>
              <p className="text-xs text-slate-400 mt-2 font-bold">{stateData?.taxRate || '7% excise'} state cannabis tax</p>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
              ...
              <p className="text-4xl font-black text-white">12,847</p>
              <p className="text-xs text-slate-400 mt-2 font-bold">Direct: 8,420 • Indirect: 4,427</p>
              <div className="mt-4 h-2 bg-white/5 rounded-full overflow-hidden">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
        <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-6 flex items-center gap-2">
          <Banknote size={18} className="text-emerald-600" /> Tax Revenue Allocation — Where the Money Goes
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { dest: 'Public Education', pct: '40%', amount: '$59.3M', icon: GraduationCap, color: 'bg-blue-50 border-blue-200 text-blue-700' },
            { dest: 'Drug Courts & Treatment', pct: '25%', amount: '$37.1M', icon: Scale, color: 'bg-amber-50 border-amber-200 text-amber-700' },
            { dest: `${stateData?.licensingAuthority || 'OMMA'} Operations`, pct: '20%', amount: '$29.6M', icon: Landmark, color: 'bg-emerald-50 border-emerald-200 text-emerald-700' },
            { dest: 'General Fund', pct: '15%', amount: '$22.2M', icon: Briefcase, color: 'bg-violet-50 border-violet-200 text-violet-700' },
          ].map((item, i) => (
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
        <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-6 flex items-center gap-2">
          <Users size={18} className="text-blue-600" /> Jobs Created by Sector
        </h3>
        <div className="space-y-4">
          {[
            { sector: 'Retail / Dispensaries', jobs: 3840, pct: 30, color: 'bg-emerald-500' },
            { sector: 'Cultivation & Growing', jobs: 2826, pct: 22, color: 'bg-green-500' },
            { sector: 'Processing & Manufacturing', jobs: 1928, pct: 15, color: 'bg-blue-500' },
            { sector: 'Testing & Laboratories', jobs: 1285, pct: 10, color: 'bg-cyan-500' },
            { sector: 'Transportation & Distribution', jobs: 1028, pct: 8, color: 'bg-amber-500' },
            { sector: 'Compliance & Legal', jobs: 771, pct: 6, color: 'bg-violet-500' },
            { sector: 'Ancillary Services (Tech, Consulting)', jobs: 1169, pct: 9, color: 'bg-indigo-500' },
          ].map((job, i) => (
            <div key={i} className="flex items-center gap-4">
              ...
              <span className="text-sm font-black text-slate-800 w-16 text-right">{job.jobs.toLocaleString()}</span>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-700/50">
                <th className="text-left py-3 px-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">State</th>
                <th className="text-right py-3 px-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Status</th>
                <th className="text-right py-3 px-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Licenses</th>
                <th className="text-right py-3 px-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Tax Rate</th>
                <th className="text-right py-3 px-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Revenue</th>
                <th className="text-right py-3 px-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Reciprocity</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/50">
              {[
                { state: 'Oklahoma', status: 'Medical Only', licenses: '4,281', tax: '7% excise', revenue: '$148.2M', reciprocity: true, highlight: true },
                { state: 'Colorado', status: 'Med & Rec', licenses: '3,847', tax: '15% + 15%', revenue: '$423.5M', reciprocity: false },
                { state: 'Missouri', status: 'Med & Rec', licenses: '2,104', tax: '6%', revenue: '$210.8M', reciprocity: true },
                { state: 'Arkansas', status: 'Medical Only', licenses: '142', tax: '6.5% + 4%', revenue: '$42.1M', reciprocity: false },
                { state: 'Texas', status: 'CBD/Hemp Only', licenses: '3', tax: 'N/A', revenue: '<$1M', reciprocity: false },
                { state: 'Kansas', status: 'CBD/Hemp Only', licenses: '0', tax: 'N/A', revenue: '$0', reciprocity: false },
              ].map((s, i) => (
                <tr key={i} className={cn("hover:bg-white/5 transition-colors", s.highlight && "bg-emerald-500/5 border-l-2 border-l-emerald-500")}>
                  <td className="py-3 px-4 font-bold text-white">{s.state} {s.highlight && <span className="text-[8px] font-black text-emerald-400 uppercase ml-2">Current</span>}</td>
      ...
      <div className="bg-gradient-to-r from-violet-50 to-indigo-50 border border-violet-200 rounded-2xl p-8">
        <h3 className="text-sm font-black text-violet-800 uppercase tracking-widest mb-6 flex items-center gap-2">
          <BadgeCheck size={18} /> Social Equity & Community Impact
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Minority-Owned Licenses', value: '812', sub: '19% of total' },
            { label: 'Rural Economic Zones', value: '34', sub: 'Active across 22 counties' },
            { label: 'Veteran-Owned Businesses', value: '186', sub: '4.3% of licensees' },
            { label: 'Social Equity Fund', value: '$4.2M', sub: 'Grants dispersed (FY26)' },
          ].map((eq, i) => (
  ...
  // ═══════════════════════════════════════════════════════════════
  //  TAB: LICENSE COMMAND — Approvals / Denials (KEPT)
  // ═══════════════════════════════════════════════════════════════
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-slate-50">
            {[
              ...
              { n: 'Prairie Wind Processing', e: 'info@prairiewind.com', t: 'Processor License', r: 'Stillwater', st: 'Pending' },
            ].map((app, i) => (
              <tr key={i} className="hover:bg-slate-50 transition-colors group">
                ...
                <td className="px-8 py-6 text-right">
                  <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => { import('../lib/turso').then(({ turso }) => turso.execute({ sql: "INSERT INTO audit_logs (id, action, user_id, data) VALUES (?, ?, ?, ?)", args: ['log-' + Math.random().toString(36).substr(2, 9), "LICENSE_APPROVE", "STATE_User", JSON.stringify({ detail: "Application approved." })] }).catch(console.error)); alert("Application approved. License authorization issued."); }} className="px-4 py-2 bg-emerald-600 text-white text-[10px] font-black rounded-xl hover:bg-emerald-500 uppercase tracking-widest">Approve</button>
  ...
  // ═══════════════════════════════════════════════════════════════
  //  TAB: COMPLIANCE PULSE — Real Content (replaces placeholder)
  // ═══════════════════════════════════════════════════════════════
  const renderCompliancePulse = () => (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
        ...
        <p className="text-slate-500 text-sm mb-8">Real-time compliance tracking across all {stateData?.licensingAuthority || 'OMMA'} licensed entities.</p>
        
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Total Inspections (YTD)', value: '1,842', icon: CheckCircle2, color: 'text-emerald-600', bg: 'bg-emerald-50 border-emerald-200' },
            { label: 'Violations Issued', value: '247', icon: AlertTriangle, color: 'text-amber-600', bg: 'bg-amber-50 border-amber-200' },
            { label: 'Licenses Suspended', value: '18', icon: XOctagon, color: 'text-red-600', bg: 'bg-red-50 border-red-200' },
            { label: 'Compliance Rate', value: '96.8%', icon: ShieldCheck, color: 'text-indigo-600', bg: 'bg-indigo-50 border-indigo-200' },
          ].map((stat, i) => (
        ...
        <div className="space-y-3">
          {[
            { category: 'Metrc Reporting Discrepancies', count: 84, pct: 34, color: 'bg-amber-500' },
            { category: 'Packaging / Labeling Non-Compliance', count: 52, pct: 21, color: 'bg-orange-500' },
            { category: 'Security Camera Requirements', count: 41, pct: 17, color: 'bg-red-500' },
            { category: 'Waste Disposal Protocols', count: 32, pct: 13, color: 'bg-violet-500' },
            { category: 'Operating Hours Violations', count: 22, pct: 9, color: 'bg-blue-500' },
            { category: 'Other', count: 16, pct: 6, color: 'bg-slate-400' },
          ].map((v, i) => (
            <div key={i} className="flex items-center gap-4">
              ...
              <span className="text-sm font-black text-slate-800 w-10 text-right">{v.count}</span>
      ...
      <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
        <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-4 flex items-center gap-2"><Clock size={16} className="text-slate-600" /> Recent Enforcement Actions</h3>
        <div className="space-y-3">
          {[
            { entity: 'Cloud Nine Dispensary', action: 'License Suspended — 30 days', reason: 'Failure to report 3 consecutive Metrc manifests', date: 'Jul 12, 2026', severity: 'high' },
            { entity: 'Harvest Moon Farms', action: 'Written Warning', reason: 'Security camera blind spot identified during inspection', date: 'Jul 10, 2026', severity: 'medium' },
            { entity: 'PureLeaf Labs', action: 'Fine — $2,500', reason: 'COA not uploaded within 72-hour window', date: 'Jul 8, 2026', severity: 'medium' },
            { entity: 'GreenState Processing', action: 'Corrective Action Plan Required', reason: 'Waste disposal not documented per OAC 681:10-7-22', date: 'Jul 5, 2026', severity: 'low' },
          ].map((e, i) => (
  ...
  // ═══════════════════════════════════════════════════════════════
  //  TAB: OMES ENTERPRISE VIEW — Integration Map
  // ═══════════════════════════════════════════════════════════════
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-gradient-to-br from-[#0a1628] to-[#1a2744] p-8 rounded-3xl border border-indigo-500/20 text-white relative overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-3xl font-black tracking-tight mb-2">GGP-OS × OMES Integration Map</h2>
          <p className="text-slate-400 font-medium max-w-3xl">How the Global Green Hybrid Platform Operating System maps to Oklahoma's Office of Management & Enterprise Services divisions — enabling a single state technology contract for comprehensive cannabis industry oversight.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {[
          ...
          { division: 'Information Services (OMES IS)', desc: 'State IT infrastructure, data hosting, cybersecurity standards', ggpModule: 'Platform Architecture & CEYE', icon: Cpu, color: 'border-cyan-200 bg-cyan-50', iconColor: 'text-cyan-600', capabilities: ['SOC 2 Type II compliant hosting', 'State data residency requirements', 'Role-based access control (RBAC)', 'Real-time threat monitoring (CEYE)'] },
          { division: 'Financial Services', desc: 'Tax collection, fee processing, state revenue reporting', ggpModule: 'Accounting Ledger & Revenue Engine', icon: DollarSign, color: 'border-emerald-200 bg-emerald-50', iconColor: 'text-emerald-600', capabilities: ['Automated fee collection & receipts', 'Tax revenue dashboarding', 'Financial audit trail (immutable)', 'Integration with state treasury'] },
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-violet-600 p-8 rounded-2xl text-white relative overflow-hidden shadow-xl">
        ...
        <div className="relative z-10">
          <h3 className="text-xl font-black mb-4">State Contract Value Proposition</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { metric: 'Projected Annual Savings', value: '$8.2M', desc: 'Versus maintaining 4+ separate legacy systems' },
              ...
              <div key={i} className="bg-white/10 backdrop-blur-sm rounded-xl p-5 border border-white/10">
                <p className="text-[10px] font-black text-white/60 uppercase tracking-widest mb-1">{m.metric}</p>
                <p className="text-3xl font-black text-white">{m.value}</p>
                <p className="text-xs text-white/70 mt-2 font-medium">{m.desc}</p>
              </div>
            ))}
          </div>
        </div>
  ...
  const renderCrossAgency = () => (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
        ...
        <p className="text-slate-500 text-sm mb-8">How cannabis regulatory data flows between state agencies for coordinated oversight and enforcement.</p>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {[
            { agency: stateData?.licensingAuthority || 'OMMA', role: 'Licensing, Compliance, Patient Registry', icon: Leaf, color: 'bg-emerald-50 border-emerald-200 text-emerald-700', flows: ['Sends license data → Tax Commission', 'Receives background checks ← OSBI', 'Sends violation reports → AG Office'] },
            { agency: stateData?.alcoholAuthority || 'ABLE Commission', role: 'Co-located license verification', icon: Store, color: 'bg-amber-50 border-amber-200 text-amber-700', flows: ['Cross-references dispensary locations', 'Shares proximity violation alerts', 'Joint inspection coordination'] },
            { agency: 'OK Tax Commission', role: 'Revenue collection & reporting', icon: Receipt, color: 'bg-blue-50 border-blue-200 text-blue-700', flows: ['Receives 7% excise from sales', 'Validates business tax compliance', 'Reports to state treasury'] },
            { agency: 'Dept. of Agriculture (ODAFF)', role: 'Hemp program & cultivation oversight', icon: Factory, color: 'bg-green-50 border-green-200 text-green-700', flows: ['Manages hemp licenses separately', 'Coordinates with OMMA on dual licenses', 'Pesticide testing standards'] },
            { agency: stateData?.pharmaBoard || 'OK Board of Pharmacy', role: 'Medical product standards', icon: FlaskConical, color: 'bg-cyan-50 border-cyan-200 text-cyan-700', flows: ['Product safety & dosage standards', 'Provider credentialing support', 'Pharmacovigilance data'] },
            { agency: 'OSBI', role: 'Background checks & investigations', icon: ShieldAlert, color: 'bg-red-50 border-red-200 text-red-700', flows: ['Processes owner/operator backgrounds', 'Criminal history verification', 'Investigative referrals from OMMA'] },
            { agency: 'Metrc (Franwell)', role: 'Seed-to-sale tracking system', icon: Database, color: 'bg-violet-50 border-violet-200 text-violet-700', flows: ['Real-time plant/product tracking', 'Manifest verification for transport', 'Inventory reconciliation alerts'] },
            { agency: 'Attorney General', role: 'Enforcement & legal authority', icon: Gavel, color: 'bg-slate-100 border-slate-200 text-slate-700', flows: ['Receives enforcement referrals', 'Prosecutes illegal operations', 'Consumer protection oversight'] },
          ].map((a, i) => (
              ...
              <ul className="space-y-1.5">
                {a.flows.map((f, j) => (
                  <li key={j} className="text-[10px] text-slate-600 font-medium flex items-start gap-1.5">
                    <ArrowRight size={10} className="shrink-0 mt-0.5 opacity-50" />
                    {f}
                  </li>
                ))}
              </ul>
      ...
      {/* INTERAGENCY ALERT FEED */}
      <div className="bg-slate-900 border border-slate-700 rounded-2xl p-8 text-white">
        <h3 className="text-sm font-black text-indigo-400 uppercase tracking-widest mb-6 flex items-center gap-2">
          <Zap size={16} /> Interagency Alert Feed — Live
        </h3>
        <div className="space-y-3">
          {[
            { from: 'OSBI', to: stateData?.licensingAuthority || 'OMMA', msg: 'Background check flagged — Applicant #APP-92104 has prior felony conviction requiring review', time: '2h ago', severity: 'high' },
            { from: stateData?.licensingAuthority || 'OMMA', to: 'OK Tax Commission', msg: 'New dispensary license issued — GreenMile OKC (License #GAAA-7201-R) — tax registration required', time: '4h ago', severity: 'info' },
            { from: 'Metrc', to: stateData?.licensingAuthority || 'OMMA', msg: 'Inventory discrepancy alert — Harvest Moon Farms reports 12.4 lbs unaccounted in manifest #MT-44021', time: '6h ago', severity: 'medium' },
            { from: stateData?.alcoholAuthority || 'ABLE Commission', to: stateData?.licensingAuthority || 'OMMA', msg: 'Proximity violation — New dispensary application within 1,000ft of licensed liquor establishment', time: '1d ago', severity: 'medium' },
            { from: 'Attorney General', to: stateData?.licensingAuthority || 'OMMA', msg: 'Enforcement action complete — Illegal operation in Payne County shut down, 3 arrests', time: '2d ago', severity: 'info' },
          ].map((alert, i) => (
            <div key={i} className={cn("p-4 rounded-xl border flex items-start gap-4", alert.severity === 'high' ? 'bg-red-500/5 border-red-500/20' : alert.severity === 'medium' ? 'bg-amber-500/5 border-amber-500/20' : 'bg-white/5 border-white/10')}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">
                  <span className="text-indigo-400">{alert.from}</span>
                  <ArrowRight size={10} />
                  <span className="text-emerald-400">{alert.to}</span>
  ...
  const renderCommerceSupplyChain = () => (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-gradient-to-br from-slate-900 to-emerald-950 p-8 rounded-3xl border border-emerald-500/20 text-white shadow-2xl">
        <h2 className="text-2xl font-black mb-2 uppercase tracking-tight">Seed-to-Sale Commerce Pipeline</h2>
        <p className="text-slate-400 text-sm mb-8">End-to-end supply chain tracked via {stateData?.traceabilitySystem || 'Metrc'} integration</p>

        <div className="flex flex-wrap items-center justify-center gap-3 mb-8">
          {[
            { stage: 'Seed', icon: '🌱', count: '428', label: 'Cultivators' },
            { stage: 'Grow', icon: '🌿', count: '1.2M', label: 'Plants Tracked' },
            { stage: 'Process', icon: '⚗️', count: '812', label: 'Processors' },
            { stage: 'Test', icon: '🔬', count: '42', label: 'Labs' },
            { stage: 'Transport', icon: '🚛', count: '1,204', label: 'Manifests/mo' },
            { stage: 'Retail', icon: '🏪', count: '2,104', label: 'Dispensaries' },
            { stage: 'Patient', icon: '👤', count: '218K', label: 'Cardholders' },
          ].map((s, i) => (
      ...
      {/* BUSINESS STATS BY CATEGORY */}
      <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
        <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-6 flex items-center gap-2">
          <Building2 size={18} className="text-indigo-600" /> Active Businesses by License Category
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            { type: 'Dispensaries', count: 2104, growth: '+4.2%', icon: Store, color: 'text-emerald-600 bg-emerald-50 border-emerald-200' },
            { type: 'Cultivators', count: 428, growth: '-8.1%', icon: Leaf, color: 'text-green-600 bg-green-50 border-green-200' },
            { type: 'Processors', count: 812, growth: '+2.3%', icon: Factory, color: 'text-blue-600 bg-blue-50 border-blue-200' },
            { type: 'Labs', count: 42, growth: '+0%', icon: FlaskConical, color: 'text-cyan-600 bg-cyan-50 border-cyan-200' },
            { type: 'Transporters', count: 310, growth: '+12%', icon: Truck, color: 'text-amber-600 bg-amber-50 border-amber-200' },
            { type: 'Waste Disposal', count: 85, growth: '+6%', icon: Activity, color: 'text-violet-600 bg-violet-50 border-violet-200' },
          ].map((biz, i) => (
            <div key={i} className={cn("p-4 rounded-2xl border text-center", biz.color)}>
              <biz.icon size={22} className="mx-auto mb-2 opacity-60" />
              <p className="text-2xl font-black">{biz.count.toLocaleString()}</p>
              <p className="text-[10px] font-bold mt-1">{biz.type}</p>
              <p className={cn("text-[10px] font-black mt-2", biz.growth.startsWith('+') ? 'text-emerald-600' : biz.growth.startsWith('-') ? 'text-red-600' : 'text-slate-400')}>{biz.growth} QoQ</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
          <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-6">Market Price Averages</h3>
          <div className="space-y-4">
            {[
              { product: 'Flower (Top Shelf)', price: '$12.40/g', trend: '↓ -3.2%', trendColor: 'text-red-500' },
              { product: 'Concentrates', price: '$28.50/g', trend: '↑ +1.8%', trendColor: 'text-emerald-500' },
              { product: 'Edibles (100mg)', price: '$14.20', trend: '↓ -1.4%', trendColor: 'text-red-500' },
              { product: 'Cartridges (1g)', price: '$32.00', trend: '↑ +0.6%', trendColor: 'text-emerald-500' },
              { product: 'Topicals', price: '$24.80', trend: '→ 0%', trendColor: 'text-slate-400' },
            ].map((p, i) => (
              <div key={i} className="flex items-center justify-between p-3 border border-slate-100 rounded-xl hover:bg-slate-50 transition-colors">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className={cn("text-xs font-bold", p.trendColor)}>{p.trend}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
          <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-6 flex items-center gap-2">
            <Database size={16} className="text-violet-600" /> {stateData?.traceabilitySystem || 'Metrc'} Sync Status
          </h3>
          <div className="space-y-4">
            {[
              ...
              { metric: 'Manifests Today', value: '1,204', status: 'Normal', statusColor: 'text-emerald-600 bg-emerald-50' },
              { metric: 'Failed Syncs (24h)', value: '3', status: 'Low', statusColor: 'text-amber-600 bg-amber-50' },
              ...
              <div key={i} className="flex items-center justify-between p-3 border border-slate-100 rounded-xl">
                <span className="text-sm font-bold text-slate-600">{m.metric}</span>
                <div className="flex items-center gap-3">
                  <span className="text-sm font-black text-slate-800">{m.value}</span>
                  <span className={cn("text-[9px] font-black uppercase px-2 py-0.5 rounded-lg", m.statusColor)}>{m.status}</span>
                </div>
              </div>
            ))}
          </div>
  ...
  // ═══════════════════════════════════════════════════════════════
  //  TAB: STATE REVENUE & TAX (replaces placeholder)
  // ═══════════════════════════════════════════════════════════════
  const renderRevenueTax = () => (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-gradient-to-br from-emerald-900 to-teal-900 p-8 rounded-3xl border border-emerald-500/30 text-white">
        <h2 className="text-3xl font-black tracking-tight mb-2">State Revenue & Tax Dashboard</h2>
        <p className="text-emerald-200/70 font-medium">Cannabis tax collection, fee revenue, and allocation tracking for {stateJurisdiction}.</p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
          {[
            { label: 'YTD Tax Revenue', value: '$84.6M', sub: '57% of annual projection' },
            { label: 'Monthly Average', value: '$12.1M', sub: '+4.2% vs last year' },
            { label: 'License Fee Revenue', value: '$18.4M', sub: '4,281 active licenses' },
            { label: 'Penalty Collections', value: '$1.2M', sub: '247 violations enforced' },
          ].map((kpi, i) => (
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
        ...
        <div className="space-y-3">
          {[
            { month: 'January', revenue: '$11.2M', pct: 76 },
            { month: 'February', revenue: '$10.8M', pct: 73 },
            { month: 'March', revenue: '$12.4M', pct: 84 },
            { month: 'April', revenue: '$13.1M', pct: 89 },
            { month: 'May', revenue: '$12.8M', pct: 87 },
            { month: 'June', revenue: '$12.6M', pct: 85 },
            { month: 'July', revenue: '$11.7M', pct: 79 },
          ].map((m, i) => (
            <div key={i} className="flex items-center gap-4">
              <span className="text-xs font-bold text-slate-500 w-20 shrink-0">{m.month}</span>
              <div className="flex-1 h-4 bg-slate-100 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-emerald-500 to-teal-400 rounded-full transition-all duration-1000" style={{ width: `${m.pct}%` }}></div>
              </div>
              <span className="text-sm font-black text-slate-800 w-20 text-right">{m.revenue}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  const navItems = [
    ...
    { section: 'Interagency' },
    { id: 'omes_enterprise', label: 'OMES Enterprise View', icon: Layers, tier: 'pro' },
    { id: 'cross_agency', label: 'Cross-Agency Coordination', icon: Network, tier: 'pro' },
    { section: 'Industry' },
    { id: 'commerce_supply', label: 'Commerce & Supply Chain', icon: Truck, tier: 'basic' },
    { id: 'health_labs', label: 'Health & Labs', icon: FlaskConical, tier: 'basic' },
    { id: 'revenue_tax', label: 'State Revenue & Tax', icon: DollarSign, tier: 'pro' },
    { section: 'System' },
      ...
      <div className={cn("w-full md:w-64 bg-[#1a4731] text-white flex flex-col shrink-0 transition-all duration-500", !isUnlocked && "blur-md opacity-50 pointer-events-none")}>
        <div className="p-6 pb-2">
          ...
          <div className="mb-4 px-3 py-2 bg-white/5 border border-white/10 rounded-xl flex items-center gap-2">
            ...
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Powered by OMES Enterprise Services</span>
          </div>
        ...
        <div className="p-4 mx-4 mb-4 bg-white/5 border border-white/10 rounded-2xl">
          <p className="text-[10px] text-slate-500 leading-relaxed font-bold italic">
            🔒 Scoped Authority: Operating under Global Green oversight framework.
          </p>
        </div>
      </div>
      ...
      <div className={cn("flex-1 flex flex-col h-[calc(100vh)] overflow-hidden transition-all duration-500", !isUnlocked && "blur-md scale-[0.98] opacity-50 pointer-events-none")}>
        <div className="h-16 border-b border-slate-200 flex items-center justify-between px-10 bg-white shrink-0">
          <h1 className="text-lg font-black text-slate-900 tracking-tight uppercase">
            {navItems.find((n: any) => n.id === activeTab)?.label || activeTab.replace(/_/g, ' ')}
          </h1>
          <div className="flex items-center gap-4">
              ...
              return (
                <div className="h-full flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-slate-500 max-w-md mb-8">
                    The <strong>{currentTab.label}</strong> module is restricted to the <span className="capitalize text-indigo-600 font-bold">{currentTab.tier}</span> tier. Upgrade your state's authority subscription to unlock this module.
                  </p>
              ...
              <div className="grid grid-cols-2 gap-8">
                <div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div><p className="text-[10px] font-bold text-slate-400 uppercase">Address</p><p className="text-sm font-bold text-slate-800">123 Commerce St, {selectedApplicant.r}, OK 73102</p></div>
                  </div>
                </div>
                <div>
                  <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Application Details</h4>
                  <div className="space-y-4">
                    <div><p className="text-[10px] font-bold text-slate-400 uppercase">Application ID</p><p className="text-sm font-bold text-slate-800">APP-849201</p></div>
                    ...
                    <div><p className="text-[10px] font-bold text-slate-400 uppercase">Background Check</p><div className="flex items-center gap-1 text-emerald-600 text-sm font-bold mt-1"><CircleCheck size={16} /> Passed (OSBI)</div></div>
                  </div>
                </div>
              </div>
              <div className="mt-8">
