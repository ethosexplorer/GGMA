...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    // 100% Human Override - bypass all AI, route directly to Extension 101
    ...
    console.error('CRITICAL VOICE WEBHOOK ERROR:', error);
    const errTwiml = new VoiceResponse();
    errTwiml.say({ voice: 'Polly.Joanna-Neural' }, "Emergency System Override. Routing you directly to a human agent now.");
    const dial = errTwiml.dial({ timeout: 60, answerOnBridge: true });
    dial.client('GGMA_User');
    res.setHeader('Content-Type', 'text/xml');
    return res.status(200).send(errTwiml.toString());
  }
}
