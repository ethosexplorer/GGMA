...
// The actions are currently just <button>{action}</button>. I need to add onClick={() => alert(...)} to them.
...
const oldExposure = /<div className="mt-8 w-full max-w-2xl bg-slate-50 rounded-xl border border-slate-200 h-64 flex items-center justify-center relative overflow-hidden">[\s\S]*?<\/div>\s*<\/div>/;
